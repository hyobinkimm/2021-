...
void download() {
	WinExec("dist\\download_card.exe", SW_SHOW);
}

void upload() {
	WinExec("dist\\upload_card.exe", SW_SHOW);
}
...
public:
	...
	void SaveScore(int count) {
		...
		for (int i = 0; i < cnt; i++) { // 기존 사용자
			if (p[i].name == name) { 
				...
				if (score >= p[i].high_score) {
					...
					p[i].playing_time = double(end) - double(start);
					isNew = false;
					break;
				}
			}
		...
		if (isNew == true) { // 신규 사용자
			...
			p[cnt].playing_time= double(end) - double(start);
			cnt++;
		}
		WriteFile();
		upload();
